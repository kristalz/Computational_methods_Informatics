...
There are variables that could in principle be statistically predicted from other variables. For instance, repsonses for different physical health conditions could predict the participants' usages of medications: Problems of injuries [from `INJURY1 to `INJURY6`] and depression [from `DEP_1` to `DEP_3`] can be used to predict usages of different opioids for acute or chronic pain[from vairables `OPIOID1` to `OPIOID9`] and to see if there are any associations in between those variables. Participations' age (`AGE`), levels of physical activities (e.g,`MODLNGNO_UNIT`), employment status (`EMPLOY`), and income (`INCOME`) can also be used to predict healthcare conditions (e.g, chronic health conditions like hyptertensions [`HYPDIF_A`] and diebetes [`DIBAGE_A`]). 
