...
5. In both cases, I got quiet different results compared to using only one hash function using different hash functions to find the median hash value. Generally, there were more variations in the estimated values by using only one hash function than those of the medians from different subsets of hash functions. I realized that the estimation would be more accurate (or smoother) using different subsets of hash functions compared to only using one. As the size of dataset increased (earlier I used the a fake sequence that only had 10000 elements and then here the entire sequence), there were also more variations between the estimated values and the actual value. Greater accuracies were observed in the smaller/median sized dataset when using larger size of hash functions.
...
All groups (i.e., age, race, sex, education level, urbanization, and selected chronic conditions) can serve as predictors for the variables "Response" of using telemedicine during COVID. For example, level of education can determine literacy in technology and healthcare: Those who with lower educational levels might have lower technological competency and might not even have the purchase power for computer/smart phones. Age is also a factor for technological literacy and openness: Seniors might be reluctant to use telemedicine. Additionally, urbanization can affect access to healthcare. Those who live in a rural areas might use telemedicine more frequently. People who have history of chronic disorders also more likely use telemedicine to monitor conditions. There are total of 126 row in age and education level groups, 168 rows in race and chronic condition groups, as well as 84 rows in sex and urbanization groups. 

The data is in a standard format because each element is in their atomic form in a separate cell. For instance, the number of people responding using telemedicine in "18-44 years" are separated from those who age in "65 years and over". However，there are lots of `NaN` in the datasets, which are not helpful for analysis. I would need to drop those values during my data cleaning process. 
