...
Step 2: I used a for loop to find all 15-mers subsequences of 15 bases within chromosome 2 in the range of the entire length of `sequence` after subtracting the last 14 indexes. (Reference: https://www.biostars.org/p/14834/). I subtracted 14 from the length of sequence because the last 14 subsequences did not contrain 15 bases. There were a total of 242193515 (potentially duplicated) subsequences.

Step 3: Find the number of total subsequences (counting duplicates) that do not contain more than 2 Ns. I made a for loop in the range of the subsequence and used the `if` condition to count numbers sequences with less than 2 'n's (I used `count`b'n because the sequence was encoded). There were a total of 240548031 subsequences that do not contain more than 2 Ns.
...
1. Before testing the entire chromosome 2, I used the 1-hash function to find the minimal hash number for the first 15000000 of the sequence. I pass 1 into the function `get_ath_hash` as `first_hash` and passed the first 15000000 of the sequence ([0:15000000]) into the function `first_hash`. I got a hash value `23262025092` for this portion of sequence. The distinct value I found by using `len` function over the `set` of this test sequence was `13178676`(`set` does not contain duplicated elements). Obviously, hashing only one times would not get the minimal distinct value. Therefore, I used a for loop that went through the entire test sequence, compared each newly found minimal hash value to the hash number for the first 15-mer subsequence to see which one was smaller, untill I got the smallest result. I got `13349` as my minimal hash value. To convert this number on a range from 0 to 1, I divided the result by 34359738367 (I printed out this hexadecimal scale number to a decimal number) and got `3.8850703277824525e-07`. After that, I calculated my minimal distinguished number by subtracting 1 from the division of 1 over my minimal hash number. Thus, I got my minimal distinct number `2573955`(I rounded to the whole number). 
2. Then I estimated the minimal hash number on the entire list of 15-mer subsequence using the 1-hash function. Using a for loop that went through the entire list, I compared each newly found minimal hash value to the hash number for the first 15-mer subsequence to see which one was smaller, untill I got the smallest result. At the end, I got 520 as my new minimal hash number,`1.5133991837941983e-08` as the hash value on a scale from 0 to 1, and `66076419` as the distinct value. 
3. Next I used a = 100 to estimate the  minmal hash number by looping through the entire list of 15-mer subsequence. My result were `80` for minimal hash value, `2.328306436606459e-09` on a scale from 0 to 1, and `429496729` as minimal distinguished number. However,the distinct value I found by using `len` function over the `set` of `subseq` was `145003145`. Using only one hash function (either a=1, a=10, or a=100) lack the accuracy to estimate the distinct values. Therefore, I modified my function that would taken different ranges of hash function as described below. 

Step 5: Compare my estimate change for different-sized subsets of these hash functions

1. I first decided to test the hash families on the first 1000000 of the sequence, which I stored in a `test` list. I created a function `find_min_hash_median` that takes `a` and a `sequence` list to loop through the entire list to find out the minimal hash value. Again, I used the hash number for the first 15-mer subsequence as my initial minimal hash value. Inside the function, the outer for loop would go through a number of hash families from 1 to a, and the inner for loop would continue until finding the smallest hash value. I tested my function using only a=1 hash function, the median of 10 and the median of 100 total hash functions and got `37244`, `29120.0` and `23099.0` as my minimal median hash value, `1.083943061562137e-06`, `8.47503542924751e-07` and `6.722693797396574e-07` minimal hash values divided by scale, and `922556`, `1179935`and `1487498`as my minimal distinguished values. The actual distinct value for this test data set was 931157, and thus closer to the distinct value found by using only the 1- hash function. 
2. I also generated fake genome sequence using `random` function that consisted of 100000 different nucleic acids `['a','c','t','g','n']`. I made a for loop to join each individual numbers into the entire sequence. I encoded the sequence in `'utf8'` to ensure the sequence can be tested in the function. Then, I subsetted `fake_subseq` for the fake sequence into each 15-mer as a subsequence. I used 1 hash function to estimate the distinct value by the same method in the previous step. `16506` was my result. I also used the median of 10 and 100 hash functions to estimate the distinct values. I compared my results above with the actual distinct value `16554` and found that using 1-hash funciton would return the closest value. 
3. According to the two sets of test above, I could convince myself that the function had the potention to find the minimal distinct value, but there were quite a bit variations using different subsets of hash functions. 
4. After the testing above, I used the function I created in the previous step and repeated the same process. But this time, I called the function to loop through the actural chronmosome 2 using different-sized subsets in 1,5,10, and 100.  
5. Using 1 hash functions, I got `520` as my minimal hash value, `1.5133991837941983e-08` minimal hash value divided by scale, and `66076419` for my minimal distinguished value. 
6. Using 5 hash functions, I got `177` as my median minimal hash value, `5.15137799099179e-09` minimal hash value divided by scale, and `194122815` for my minimal distinguished value. 
7. Using 10 hash functions, I got `247.5` as my minimal hash value, `7.203198038251232e-09` minimal hash value divided by scale, and `138827225` for my minimal distinguished value. 
8. Using 100 hash functions, I got `170.5` as my minimal hash value, `4.9622030930175156e-09` on a scale from 0 to 1, and `201523391` for my minimal distinguished number.
9. Using different hash functions to find the median hash value, I got quiet different results compared to using only one hash function. Generally, the hash value from only one hash function (a=100) was much larger than the medians from different subsets of hash functions. 
10. Earlier I found the length of a `set` of all subsequences as `145003145`, which was closer to the results using the median of 10 hash functions. Therefore, I realized that the estimation would be more accurate using different subsets of hash functions compared to only using one. As the size of dataset increased (earlier I used the first 15000000 and then here the entire sequence), there were more variations between the estimated value and the actual value. However, more accuracies were observed in the smaller/median sized dataset (the first 1000000 protion of the sequence and the fake 100000 sequence that consisted of random genomic bases I made above). 
