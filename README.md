...
The data is not in a standard format although each element is in their atomic form in a separate cell. However，there are lots of missing data (represented by `.`) in the dataset, which are not helpful for analysis. I would need to drop those values during my data cleaning process and make my dataset tidy. Moreover, the responses for each variables can be sum up in numbers or calculated in percentages. 
