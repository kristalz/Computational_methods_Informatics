...
6. I printed out the estimated values using 1-hash, mean of 10 hash functions, and mean of 100 hash functions for both tests. Using the mean of 10 hash functions to estimate the distinct value were more accurate in the first test (100000), whereas using the mean of 100 hash functions were closer to the actual value in the second test(10000). I wondered what the results would be in chromosome2.
...
5. In all cases above, I got quiet different results compared to using only one hash function using different hash functions to find the median hash value. Generally, there were more variations in the estimated values by using only one hash function than those of the medians from different subsets of hash functions. I realized that the estimation would be more accurate (or smoother) using different subsets of hash functions compared to only using one. As the size of dataset increased, there were also more variations between the estimated values and the actual value. In addition, greater accuracies were observed in the smaller/median (10000) rather than larger (100000 and the chromosome2) sized dataset when using larger sizes of hash functions. However, I could not tell what exactly caused using 10 hash functions to estimate the distinct value in 100000-based fake sequence would be more accurate, a situation that different from those in fake 10000-based sequence and chromosome2.
...
8. According to the results, I found that for the first hash, first 10 hash, and all 100 hash functions, the estimated values were `922556.683573193, 1034472.843418727, and 1048865.6405058529` for the first 1000000 and `127919.15862443828, 65453.663569801836, 83027.71125458396`, for the first 100000 portions of chromosome2, respectively. The estimated values using first hash function were closer to the actual value in first case(first 1000000), but were closesr using 100-hash funcitons in the second case (first 100000). The first case did not converge throughout the entire 100 hash functions. In the second case, however, the results did not converge until over 30 hash functions. Nevertheless, the estimated outcomes were not as great as those in my fake sequence tests, especially for the first 1000000 portions of chromosome2 sequence (the estimated outputs were quite different through out the entire hash functions). 
...
The data is not in a standard format although each element is in their atomic form in a separate cell. However，there are lots of empty cells(represented by `.` in the dataset) in the datasets, which are not helpful for analysis. I would need to drop those values during my data cleaning process and make my dataset tidy. Moreover, the responses for each variables can be sum up in numbers or calculated in percentages. 

There are variables that could in principle be statistically predicted from other variables. For instance, repsonses for different physical health conditions could predict the participants usages of medications: Problems of injuries (varibales `INJURY1`to `INJURY6`) and depression (variables `DEP_1` to `DEP_3`) can be used to predict usages of different opioids for acute or chronic pain in vairables OPIOID1toOPIOID9. Participations' age (`AGE`), levels of physical activities (e.g,`MODLNGNO_UNIT`), employment status (`EMPLOY`), and income (`INCOME`) can also be used to predict healthcare conditions (e.g, chronic health conditions like hyptertensions `HYPDIF_A` and diebetes `DIBAGE_A`). Associations or correlation can also be observed in the relationship among these variables.
